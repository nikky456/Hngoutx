...
const MovieRoom: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: '1', userId: 'bot', username: 'HX Bot', text: 'Welcome to the HangoutX MOVIE session! Syncing everyone now.', timestamp: new Date() },
  ]);
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    if (currentInput.toLowerCase().startsWith('/ai')) {
      ...
      const mediaContext = `${currentMedia.title} (MOVIE) by ${currentMedia.artist}`;
      ...
      <nav className="h-16 border-b border-white/5 px-6 flex items-center justify-between bg-slate-950/40 backdrop-blur-3xl z-40">
        ...
        <div className="flex items-center gap-4">
          <div className="hidden lg:flex items-center gap-2 bg-slate-900/80 border border-white/5 rounded-full px-4 py-1.5 shadow-xl">
             <span className="text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]">Movie Session</span>
